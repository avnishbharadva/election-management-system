...
export const fetchCandidates = createAsyncThunk("candidate/fetchAll", async () => {
  const response = await axios.get("http://localhost:8081/api/candidate/getAllDetails"); // Replace with your API endpoint
...
export const fetchCandidateBySSN = createAsyncThunk(
  ...
  async (candidateSSN: string, { dispatch, rejectWithValue }) => {
    try {
      const response = await axios.get(`http://localhost:8081/api/candidate/ssn/${candidateSSN}`);
      return response.data;
    } catch (error: any) {
  ...
  async (formData: FormData, { rejectWithValue }) => {
    try {
      const response = await axios.post(
        "http://localhost:8081/api/candidate/addCandidate",
        formData,
        {
          headers: {
