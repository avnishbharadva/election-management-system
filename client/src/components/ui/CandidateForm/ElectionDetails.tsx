...
const ElectionDetails = ({ control, errors, register, selectedCandidate }: any) => {
  const dispatch: AppDispatch = useDispatch();
  const elections = useSelector((state: RootState) => state.election.elections) || [];
  const [parties, setParties] = useState<{ partyId: number; partyName: string }[]>([]);
  ...
  useEffect(() => {
    if (selectedCandidate) {
      ...
      control.setValue("electionId", selectedCandidate.electionId || "");
      control.setValue("partyId", selectedCandidate.partyId || "");
    }
  ...
  const handleElectionOpen = () => {
    dispatch(fetchAllElection()); 
  };

  const handlePartyOpen = () => {
    fetchParties();
  };
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                ...
                labelId="election-label"
                label="Election Type"
                onOpen={handleElectionOpen}  // Calls only the election API
                value={field.value ?? ""}
                onChange={(event) => field.onChange(event.target.value)}
              >
                {/* Prefilled option */}
                {selectedCandidate?.electionId && !elections.some(e => e.electionId === selectedCandidate.electionId) && (
                  <MenuItem value={selectedCandidate.electionId}>
          ...
          <Controller
            ...
            render={({ field }) => (
              <Select
                ...
                labelId="party-label"
                label="Party"
                onOpen={handlePartyOpen} // Calls only the party API
                value={field.value ?? ""}
                onChange={(event) => field.onChange(event.target.value)}
              >
                {/* Prefilled option */}
                {selectedCandidate?.partyId && !parties.some(p => p.partyId === selectedCandidate.partyId) && (
                  <MenuItem value={selectedCandidate.partyId}>
